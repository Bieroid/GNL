...
static	char	*ft_trim(char **save)
{
	...
	tmp = malloc(sizeof(char) * (ft_strlen(*save) + 1 - pos));
	if (!tmp)
		return (0);
	a = 0;
	pos++;
...
int	get_next_line(int fd, char **line)
{
	...
	if (!buf)
		return (-1);
	ret = 1;
	while (ret && ft_check_newline(save[fd]) == -1)
	{
		...
		buf[ret] = '\0';
		save[fd] = ft_strjoin_new(save[fd], buf);
	}
	return (ft_result(&save[fd], line, &buf, ret));
}
